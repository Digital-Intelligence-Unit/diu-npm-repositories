...
class Credential extends BaseModel {
    ...
    create(attributes, callback) {
        // Encrypt function
        const encrypt = (data) => {
            return CryptoJs.AES.encrypt(JSON.stringify(data), process.env.JWT_SECRETKEY).toString();
        };
    ...
    getByKeys(tableKeys, callback) {
        super.getByKeys(tableKeys, (err, result) => {
            ...
            result.Items = result.Items.map((item) => {
                item.data = JSON.parse(CryptoJs.AES.decrypt(item.data, process.env.JWT_SECRETKEY).toString(CryptoJs.enc.Utf8));
                return item;
            });
