...
class GovUkModel extends BaseModel {
    ...
    update(item, callback) {
        console.log("GOV UK Notify ACK---Receipt: " + JSON.stringify(item));
        // Log status against messageid === id
        const updatequery =
            "UPDATE public.virtualward_lightertouchpathway SET status='Message " + item.status + "' WHERE messageid = '" + item.id + "'";
        this.query(updatequery, (err, res) => {
            ...
            CredentialModel(
                ...
                (errCM, result) => {
                    ...
                    const servicecountlogquery = `INSERT INTO public.virtualward_servicecountlog
                        (messageid, msgcount, organisation, servicename, type, period) VALUES ($1, $2, $3, $4, $5, $6)`;
    ...
    getAllServiceCountLogs = function (callback) {
        const query = `SELECT logs.*, pathway.status FROM public.virtualward_servicecountlog as logs
        LEFT OUTER JOIN public.virtualward_lightertouchpathway as pathway
         ON logs.messageid = pathway.messageid`;
    ...
    updateGeneral = function (service, item, callback) {
        console.log("GOV UK Notify ACK---Receipt: " + JSON.stringify(item));
        const servicecountlogquery = `INSERT INTO public.notify_callbacks
            (messageid, msgcount, organisation, servicename, type, period, templateid, status, recipient)
