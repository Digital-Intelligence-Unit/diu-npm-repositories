...
class ClinicalTrialModel extends BaseModel {
  ...
  getSearchTop1000(query, phases, min_date, callback) {
    const parsedQuery = this._extractfromCohort(query, phases, min_date);
    this.query(
      `SELECT
                s.nct_id,
                CONCAT('https://clinicaltrials.gov/ct2/show/study/', s.nct_id) AS url,
  ...
  _extractfromCohort(cohorturl, phases, min_date) {
    let builtquery = " AND completion_date >= now() - interval '" + min_date + " year' ";
    ...
    if (parsedCohort["AgeDimension"]) {
      builtquery += " AND (COALESCE(NULLIF(regexp_replace(minimum_age, '\\D','','g'), '')::numeric, 0) >= " + parsedCohort["AgeDimension"][0][0];
      builtquery += " AND COALESCE(NULLIF(regexp_replace(maximum_age, '\\D','','g'), '')::numeric, 0) <= " + parsedCohort["AgeDimension"][0][1] + ") ";
    }
